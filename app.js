...
const createScene = async function () {
        ...
        const updateOccluderScale = (scale) => {
            ...
            [occluderFloor, occluderTop, occluderRight, occluderLeft, occluderback].forEach(ocl => {
                ocl.scaling.x = scaleFactor;
                ocl.scaling.y = scaleFactor;
            });
        };
        ...
        scene.onBeforeRenderObservable.add(() => {
                ...
                if (xr.input.controllers.length > 0 && portalAppearded) {
                    ...
                    if (gamepad) {
                        ...
                        switch (portalState) {
                            case 'scaling':
                                portalScale += axes[3] * 0.02;
                                portalScale = Math.max(0.5, Math.min(2.0, portalScale));
                                rootPilar.scaling.setAll(portalScale);
                                updateOccluderScale(portalScale);
                                break;
